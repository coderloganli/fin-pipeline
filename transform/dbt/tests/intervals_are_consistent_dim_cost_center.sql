-- Gate 4, for dim_cost_center. Four failures, one test, because they are one property: the
...
with versions as (
    select
        cc_code as natural_key,
        valid_from,
        valid_to,
        lead(valid_from) over (partition by cc_code order by valid_from) as next_from
