-- Gate 4, for dim_fx_rate. Four failures, one test, because they are one property: the
...
with versions as (
    select
        currency as natural_key,
        valid_from,
        valid_to,
        lead(valid_from) over (partition by currency order by valid_from) as next_from
